...
const signUp = async (req, res) => {
    ...
    if (user) {
        try {
            ...
            console.log(error);
            res.status(500).send({ msg: "Error Fetching User Details, Try Again Later" });
        }
    } else {
        await hashPassword(req, res).then(() => {
            User.create(req.body).then(user => {
                if (user) {
                    res.status(200).send({ msg: "Registered Successfully", user: user[0] });
                } else {
                    res.status(500).send({ msg: "Error Creating User Details, Try Again Later" });
                }
            })
        });
    }
